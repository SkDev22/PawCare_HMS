...
export function DashboardPage() {
  ...
  const completedPct = data && data.stats.todayAppointments.total > 0
    ? Math.round((data.stats.todayAppointments.completed / data.stats.todayAppointments.total) * 100)
    : 0;
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Completed clinic visits over the last 6 months</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={data?.monthlyVisits ?? []} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                  <defs>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            {isLoading ? (
                  ...
                  <PieChart>
                    ...
                    >
                      {data.speciesDistribution.map((entry) => (
                        <Cell key={entry.name} fill={SPECIES_COLORS[entry.name] ?? '#a1a1aa'} />
                      ))}
                    </Pie>
                ...
                <div className="space-y-2">
                  {data.speciesDistribution.map(({ name, value }) => (
                    <div key={name} className="flex items-center gap-2">
                      <span className="size-2.5 shrink-0 rounded-full" style={{ background: SPECIES_COLORS[name] ?? '#a1a1aa' }} />
                      <span className="text-xs text-muted-foreground flex-1">{SPECIES_LABELS[name] ?? name}</span>
              ...
              <p className="text-muted-foreground text-sm py-8 text-center">No active patients yet</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                {data ? `${data.stats.todayAppointments.total} total · ${data.stats.todayAppointments.completed} completed` : ' '}
              </CardDescription>
            </div>
          ...
          <CardContent className="p-0">
            {isLoading ? (
                  ...
                  return (
                    <div key={appt.id} className="flex items-center gap-3 px-6 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium leading-none">{appt.pet.name}</p>
                        <p className="text-muted-foreground truncate text-xs mt-0.5">
                          {appt.pet.owner.first_name} {appt.pet.owner.last_name} · {APPOINTMENT_TYPE_LABELS[appt.type] ?? appt.type}
                        </p>
                      </div>
              ...
              <p className="text-muted-foreground text-sm py-8 text-center">No appointments scheduled for today</p>
            )}
            <Separator />
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {data ? `${data.stats.wardOccupancy.occupied} of ${data.stats.wardOccupancy.total} kennels occupied` : ' '}
              </CardDescription>
            </CardHeader>
              ...
              {isLoading ? (
                ...
                data.wardStatus.map((entry) => (
                  <div key={entry.id} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium leading-none">{entry.kennel.label} · {entry.pet.name}</p>
                      <p className="text-muted-foreground text-xs mt-0.5">
                        {SPECIES_LABELS[entry.pet.species] ?? entry.pet.species} · admitted {formatDistanceToNow(new Date(entry.admitted_at), { addSuffix: true })}
                      </p>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-muted-foreground text-sm py-4 text-center">No pets currently admitted</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 pt-0">
              {isLoading ? (
              ...
              ) : data && (data.alerts.lowStock.length > 0 || data.alerts.abnormalLabResults.length > 0) ? (
                <>
                    ...
                    <div key={item.id}>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground text-xs mt-0.5">
                            {item.name}: {item.quantity_on_hand} {item.unit} left
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {data.alerts.abnormalLabResults.map((result, i) => (
                    <div key={result.id}>
                      {(i > 0 || data.alerts.lowStock.length > 0) && <Separator className="mb-3" />}
                      <div className="flex items-start gap-3">
                        <FlaskConical className="size-4 text-brand-500 mt-0.5 shrink-0" />
                        <div>
                          <p className="text-sm font-medium leading-none">Abnormal result</p>
                          <p className="text-muted-foreground text-xs mt-0.5">
                            {result.lab_order.pet.name}'s {result.test_name} — {result.value}{result.unit ?? ''}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <p className="text-muted-foreground text-sm py-4 text-center">No alerts</p>
              )}
            </CardContent>
          </Card>
        </div>
