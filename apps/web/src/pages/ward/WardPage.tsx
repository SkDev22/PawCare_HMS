...
function AdmitForm({
  ...
  kennel?: KennelUnit | null;
  onSuccess: () => void;
  onCancel: () => void;
}) {
  const [petSearch, setPetSearch] = useState("");
  const [selectedPetName, setSelectedPetName] = useState("");
  const [selectedKennel, setSelectedKennel] = useState<KennelUnit | null>(
    kennel ?? null,
  );
  ...
  const availableKennels = (allKennels ?? []).filter(
    ...
    admitPet.mutate(
      {
        ...
        ...(values.estimated_stay_days
          ? { estimated_stay_days: values.estimated_stay_days }
          : {}),
      },
      { onSuccess },
    );
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                Kennel <span className="text-destructive">*</span>
              </FormLabel>
              {selectedKennel ? (
                <div className="flex items-center gap-2">
                  <div className="flex-1 rounded-md border border-input bg-muted/30 px-3 py-2 text-sm">
                    {selectedKennel.label} — {selectedKennel.room.name} (
                    {selectedKennel.size})
                  </div>
                  {!kennel && (
                    <Button
...
> = {
  ...
  OCCUPIED: {
    label: "Occupied",
    badge: "warning",
    cardClass: "border-orange-300 bg-orange-50 dark:bg-orange-950/20",
  },
  CLEANING: {
    label: "Cleaning",
    badge: "info",
    cardClass: "border-blue-300 bg-blue-50 dark:bg-blue-950/20",
  },
  OUT_OF_SERVICE: {
    label: "Out of Service",
    badge: "destructive",
    cardClass: "border-red-300 bg-red-50 dark:bg-red-950/20 opacity-80",
  },
};
...
function KennelCard({
  ...
  const clickable =
    kennel.status === "AVAILABLE" || kennel.status === "OCCUPIED";
  ...
  return (
    ...
    >
      <CardContent className="p-4">
        ...
        {canWrite &&
          (kennel.status === "CLEANING" ||
            kennel.status === "OUT_OF_SERVICE") && (
            <Button
              type="button"
              size="sm"
              variant="outline"
        ...
        <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-1">
            ...
            <div className="space-y-2">
              ...
              {wardRooms.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  Add a ward room first.
                </p>
              ) : (
                <Form {...kennelForm}>
                  <form
                    onSubmit={kennelForm.handleSubmit(onCreateKennel)}
                    className="space-y-2"
                  >
                    <FormField
                      control={kennelForm.control}
                      name="room_id"
                      render={({ field }) => (
                        <FormItem>
                          <Select
                    ...
                    <Button
                      type="submit"
                      size="sm"
                      disabled={createKennel.isPending}
                    >
                      Add Kennel
                    </Button>
                  </form>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium">
              Existing Kennels ({(kennels ?? []).length})
            </p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Kennel occupancy and inpatient care
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <Tabs
          value={tab}
          onValueChange={(v) => setTab(v as "kennels" | "list")}
        >
          <TabsList>
        ...
        (kennelsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Use "Manage Kennels" above to add ward rooms and kennels.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {filteredKennels.map((k) => (
              <KennelCard
                ...
                onMarkAvailable={() =>
                  updateKennelStatus.mutate({ id: k.id, status: "AVAILABLE" })
                }
                onMarkOutOfService={() =>
                  updateKennelStatus.mutate({
                    id: k.id,
        ...
        <Card>
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <CardTitle className="text-base font-medium">
              {hospsLoading
                ? "Loading..."
                : `${hosps.length} patient${hosps.length !== 1 ? "s" : ""}`}
            </CardTitle>
            <Button
              variant="outline"
              size="sm"
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {h.kennel.label} · {h.kennel.room.name} ·{" "}
                        {h.pet.owner.last_name}
                      </p>
                    </div>
                    <div className="text-right shrink-0 ml-4">
                      <p className="text-xs text-muted-foreground">
                        Admitted{" "}
                        {format(new Date(h.admitted_at), "MMM d, yyyy")}
                      </p>
                      {h.discharged_at && (
                        <p className="text-xs text-muted-foreground">
                          Discharged{" "}
                          {format(new Date(h.discharged_at), "MMM d, yyyy")}
                        </p>
                      )}
                    </div>
                  </div>
