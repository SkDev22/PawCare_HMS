...
function PetSearch({ value, onChange, defaultPet }: PetSearchProps) {
  ...
  const [selected, setSelected] = useState<string>(
    defaultPet
      ? `${defaultPet.name} — ${defaultPet.owner?.first_name ?? ""} ${defaultPet.owner?.last_name ?? ""}`.trim()
      : "",
  );
  ...
  function select(pet: Pet) {
    onChange(pet.id);
    setSelected(
      `${pet.name} — ${pet.owner?.first_name ?? ""} ${pet.owner?.last_name ?? ""} · ${SPECIES_LABELS[pet.species] ?? pet.species}`,
    );
    setQuery("");
    setOpen(false);
  }
...
export function AppointmentForm({
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const endAt = new Date(
      startAt.getTime() + Number(values.duration_minutes) * 60_000,
    );
