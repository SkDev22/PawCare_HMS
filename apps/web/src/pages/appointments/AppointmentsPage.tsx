...
import {
  ChevronLeft, ChevronRight, CalendarDays, Plus, List, Clock,
  PawPrint, AlertCircle,
} from 'lucide-react';
import { useCalendarView } from '@/hooks/use-appointments';
...
const TYPE_COLOR: Record<AppointmentType, string> = {
  WELLNESS_EXAM: 'bg-blue-100   border-blue-300   text-blue-800   dark:bg-blue-950   dark:border-blue-700   dark:text-blue-300',
  VACCINATION:   'bg-green-100  border-green-300  text-green-800  dark:bg-green-950  dark:border-green-700  dark:text-green-300',
  SICK_VISIT:    'bg-amber-100  border-amber-300  text-amber-800  dark:bg-amber-950  dark:border-amber-700  dark:text-amber-300',
  SURGERY:       'bg-red-100    border-red-300    text-red-800    dark:bg-red-950    dark:border-red-700    dark:text-red-300',
  DENTAL:        'bg-cyan-100   border-cyan-300   text-cyan-800   dark:bg-cyan-950   dark:border-cyan-700   dark:text-cyan-300',
  FOLLOW_UP:     'bg-purple-100 border-purple-300 text-purple-800 dark:bg-purple-950 dark:border-purple-700 dark:text-purple-300',
  EMERGENCY:     'bg-rose-100   border-rose-300   text-rose-800   dark:bg-rose-950   dark:border-rose-700   dark:text-rose-300',
  GROOMING:      'bg-pink-100   border-pink-300   text-pink-800   dark:bg-pink-950   dark:border-pink-700   dark:text-pink-300',
  LAB_ONLY:      'bg-orange-100 border-orange-300 text-orange-800 dark:bg-orange-950 dark:border-orange-700 dark:text-orange-300',
};
...
function AppointmentBlock({ appt, lane, totalLanes, onClick }: BlockProps) {
  ...
  const topPx    = (startAt.getHours() + startAt.getMinutes() / 60 - GRID_START) * HOUR_PX;
  ...
  return (
    <button
      ...
      `}
    >
      <p className="text-xs font-semibold leading-tight truncate">{appt.pet.name}</p>
      {!isTiny && (
        <>
          <p className="text-xs leading-tight truncate opacity-80">
            {appt.pet.owner.first_name} {appt.pet.owner.last_name}
          </p>
          <p className="text-xs leading-tight truncate opacity-70">{TYPE_LABEL[appt.type]}</p>
        </>
      )}
    </button>
  );
...
function CalendarGrid({ appointments, onClickAppt, selectedDate }: CalendarGridProps) {
  ...
  const currentTopPx = (now.getHours() + now.getMinutes() / 60 - GRID_START) * HOUR_PX;
  const showNow = isToday && now.getHours() >= GRID_START && now.getHours() < GRID_END;
...
function ListView({ appointments, onClickAppt }: { appointments: Appointment[]; onClickAppt: (id: string) => void }) {
  if (!appointments.length) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
        <CalendarDays className="size-12 mb-3 opacity-30" />
        <p className="font-medium">No appointments this day</p>
        <p className="text-sm mt-1">Schedule a new appointment or navigate to another date.</p>
      </div>
    );
  }
        ...
        <TableBody>
          {appointments.map((appt) => (
            ...
            >
              ...
              <TableCell>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground capitalize">{appt.pet.species.toLowerCase()}</p>
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell>
                <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${TYPE_COLOR[appt.type]}`}>
...
export function AppointmentsPage() {
  ...
  const { data: appointments = [], isLoading, isError } = useCalendarView(selectedDate);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {isLoading ? 'Loading…' : `${totalCount} total · ${completedCount} completed · ${pendingCount} pending`}
          </p>
        </div>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-border bg-background min-w-[220px] justify-center">
            ...
            {isToday && (
              <Badge variant="secondary" className="text-xs py-0 px-1.5">Today</Badge>
            )}
          </div>
          ...
          {!isToday && (
            <Button variant="ghost" size="sm" onClick={() => setSelectedDate(toDateStr(new Date()))}>
              Today
            </Button>
          )}
        </div>
      ...
      {!isLoading && !isError && view === 'calendar' && appointments.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          <CalendarDays className="size-12 mb-3 opacity-30" />
          <p className="font-medium">No appointments this day</p>
